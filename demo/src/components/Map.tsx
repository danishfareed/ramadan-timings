...
import { getDayFastingTimes, getDayPrayerTimes, formatLocalTime, formatDuration, getFastingTimesByCity } from '@danishfareed/ramadan-timings';
import type { FastingTimes, PrayerTimes } from '@danishfareed/ramadan-timings';
...
export const InteractiveMap = () => {
  ...
  useEffect(() => {
    ...
    setTimes(getDayFastingTimes(new Date(), meccaConfig));
    setPrayers(getDayPrayerTimes(new Date(), meccaConfig));
  }, []);
  ...
  const updateMapAndData = (lng: number, lat: number, name: string, offset: number) => {
    ...
    const newConfig = {
      ...
      imsakMarginMinutes: 10,
      highLatitudeMode: 'middle-of-night' as const
    };
    setTimes(getDayFastingTimes(new Date(), newConfig));
    setPrayers(getDayPrayerTimes(new Date(), newConfig));
  };
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimes(data.times);
      // Also compute prayer times for the same city using the geocoded coordinates
        ...
        <div className="results-panel mt">
          ...
          {times ? (
            <div className="times-list">
              ...
              <div className="time-row"><span>Asr:</span> <b>{prayers ? formatLocalTime(prayers.asr, offsetMinutes) : '--:--'}</b></div>
              <div className="time-row highlight-text"><span>Maghrib (End):</span> <b>{formatLocalTime(times.maghrib, offsetMinutes)}</b></div>
              <div className="time-row"><span>Isha:</span> <b>{prayers ? formatLocalTime(prayers.isha, offsetMinutes) : '--:--'}</b></div>
